...
uint8_t *csi_ctx_init(csi_csid_t suite, csi_val_t key_info, csi_svcid_t svc)
{
	...
	if(suite == CSTYPE_HMAC_SHA256)
	{
		if((context = MTAKE(32)) == NULL)
		{
			return NULL;
		}
...
int8_t  csi_sign_update(csi_csid_t suite, void *context, csi_val_t data, csi_svcid_t svc)
{
	...
	if(suite == CSTYPE_HMAC_SHA256)
	{
		...
		hmac_sha256_get(digest, data.contents, data.len, (uint8_t *)context, keyLen);

		memcpy(context, digest, 32);
...
int8_t  csi_sign_finish(csi_csid_t suite, void *context, csi_val_t *result, csi_svcid_t svc)
{
	...
	if(suite == CSTYPE_HMAC_SHA256)
	{
		...
		case CSI_SVC_SIGN:
			...
			memcpy(result->contents, context, result->len);
			return 1;

		case CSI_SVC_VERIFY:
			if(memcmp(context, result->contents, 32) == 0)
			{
				return 1;
			}
			else
