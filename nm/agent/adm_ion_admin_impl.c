...
tnv_t *dtn_ion_ionadmin_ctrl_node_contact_change(eid_t *def_mgr, tnvc_t *parms, int8_t *status)
{
	tnv_t *result = NULL;
	*status = CTRL_FAILURE;
	/*
	 * +-------------------------------------------------------------------------+
	 * |START CUSTOM FUNCTION ctrl_node_contact_change BODY
	 * +-------------------------------------------------------------------------+
	 */
	...
	if(success)
	{
		fromNodeNbr = adm_get_parm_uvast(parms, 1, &success);
	}
	
	if(success)
	{
		toNodeNbr = adm_get_parm_uvast(parms, 2, &success);
	}
	
	if(success)
	{
		xmitRate = adm_get_parm_uvast(parms, 3, &success);
	}
	
	if(success)
	{
		confidence = adm_get_parm_uvast(parms, 4, &success);
	}
	
	if(success)
	{
		if(rfx_revise_contact(fromTime, fromNodeNbr, toNodeNbr, xmitRate,
		                      confidence) == 0)
		{
			*status = CTRL_SUCCESS;
		}
	...
	/*
	 * +-------------------------------------------------------------------------+
	 * |STOP CUSTOM FUNCTION ctrl_node_contact_change BODY
	 * +-------------------------------------------------------------------------+
	 */
	return result;
}
...
tnv_t *dtn_ion_ionadmin_ctrl_node_contact_add(eid_t *def_mgr, tnvc_t *parms, int8_t *status)
{
	...
	*status = CTRL_FAILURE;
	...
	/* Temporarily sign up for the home region membership for the test */
