...
/*   START CUSTOM FUNCTIONS HERE */	/* ETRI & HYU */

/*
 * Declare that the indicated convergence-layer protocol outduct is now a viable device for
 * transmitting bundles to the node[s] identified by endpoint_name.
 */
tnv_t *dtn_ion_bpadmin_ctrl_egress_plan_outduct_attach(eid_t *def_mgr, tnvc_t *parms, int8_t *status)
{
	tnv_t *result = NULL;
	*status = CTRL_FAILURE;
	/*
	 * +-------------------------------------------------------------------------+
	 * |START CUSTOM FUNCTION ctrl_egress_plan_outduct_attach BODY
	 * +-------------------------------------------------------------------------+
	 */
	...
	char *p_name = adm_get_parm_obj(parms, 1, AMP_TYPE_STR);
	char *d_name = adm_get_parm_obj(parms, 2, AMP_TYPE_STR);

	findOutduct(p_name, d_name, &vduct, &vductElt);
	if(vductElt != 0)
	{
		if(attachPlanDuct(name, vduct->outductElt) > 0)
		{
			*status = CTRL_SUCCESS;
		}
	}
	...
	/*
	 * +-------------------------------------------------------------------------+
	 * |STOP CUSTOM FUNCTION ctrl_egress_plan_outduct_attach BODY
	 * +-------------------------------------------------------------------------+
	 */
	return result;
}
...
/*
 * Declare that the indicated convergence-layer protocol outduct is no longer a viable device for
 * transmitting bundles to the node[s] identified by endpoint_name.
 */
tnv_t *dtn_ion_bpadmin_ctrl_egress_plan_outduct_detach(eid_t *def_mgr, tnvc_t *parms, int8_t *status)
{
	tnv_t *result = NULL;
	*status = CTRL_FAILURE;
	/*
	 * +-------------------------------------------------------------------------+
	 * |START CUSTOM FUNCTION ctrl_egress_plan_outduct_detach BODY
	 * +-------------------------------------------------------------------------+
	 */
	...
	if(vductElt != 0)
	{
		if(detachPlanDuct(vduct->outductElt) > 0)
		{
			*status = CTRL_SUCCESS;
		}
	}
	...
	/*
	 * +-------------------------------------------------------------------------+
	 * |STOP CUSTOM FUNCTION ctrl_egress_plan_outduct_detach BODY
	 * +-------------------------------------------------------------------------+
	 */
	return result;
}
...
/*             TODO              */
/*   STOP CUSTOM FUNCTIONS HERE  */	/* ETRI & HYU */
