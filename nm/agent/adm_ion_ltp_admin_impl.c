...
/*
 * This control starts link service output tasks for all LTP spans (to remote engines)
 * from the local LTP engine, and it starts the link service input task for the local engine.
 */
tnv_t *dtn_ion_ltpadmin_ctrl_start(eid_t *def_mgr, tnvc_t *parms, int8_t *status)
{
	tnv_t *result = NULL;
	*status = CTRL_FAILURE;
	/*
	...
	int success;
	char *lsiCmd = adm_get_parm_obj(parms, 0, AMP_TYPE_STR);
	if(ltpStart(lsiCmd) >= 0)
	{
		*status = CTRL_SUCCESS;
	}
