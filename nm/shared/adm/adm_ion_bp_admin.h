 ...
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |name                 |4480186e00    |The human-readable name of the ADM.   |STR    |ion_bp_admin            |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |namespace            |4480186e01    |The namespace of the ADM              |STR    |DTN/ION/bpadmin         |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |version              |4480186e02    |The version of the ADM                |STR    |v0.0                    |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |organization         |4480186e03    |The name of the issuing organization o|       |                        |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |bp_version           |4482186600    |Version of installed ION BP Admin util|       |
 * |                     |              |ity.                                  |STR    |
 * +---------------------+--------------+--------------------------------------+-------+
 */
 ...
 * |        NAME         |     ARI      |             DESCRIPTION              | TYPE  |
 * +---------------------+--------------+--------------------------------------+-------+
 * |endpoints            |448a186b00    |Local endpoints, regardless of scheme |       |
 * |                     |              |name.                                 |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |inducts              |448a186b01    |Inducts established locally for the in|       |
 * |                     |              |dicated CL protocol.                  |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |outducts             |448a186b02    |If protocolName is specified, this tab|       |
 ...
 * |                     |              |col.                                  |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |protocols            |448a186b03    |Convergence layer protocols that can c|       |
 * |                     |              |urrently be utilized at the local node|       |
 * |                     |              |.                                     |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |schemes              |448a186b04    |Declared endpoint naming schemes.     |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |egress_plans         |448a186b05    |Egress plans.                         |       |
 * +---------------------+--------------+--------------------------------------+-------+
 */
 ...
 * |        NAME         |     ARI      |             DESCRIPTION              | TYPE  |
 * +---------------------+--------------+--------------------------------------+-------+
 * |endpoint_add         |44c1186500    |Establish DTN endpoint named endpointI|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |endpoint_change      |44c1186501    |Change the action taken when bundles d|       |
 * |                     |              |estined for this endpoint arrive at a |       |
 * |                     |              |time when no application has the endpo|       |
 * |                     |              |int open for bundle reception.        |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |endpoint_del         |44c1186502    |Delete the endpoint identified by endp|       |
 * |                     |              |ointId. The control will fail if any b|       |
 * |                     |              |undles are currently pending delivery |       |
 * |                     |              |to this endpoint.                     |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |induct_add           |44c1186503    |Establish a duct for reception of bund|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |induct_change        |44c1186504    |Change the control used to initiate op|       |
 * |                     |              |eration of the induct task for the ind|       |
 * |                     |              |icated duct.                          |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |induct_del           |44c1186505    |Delete the induct identified by protoc|       |
 * |                     |              |olName and ductName. The control will |       |
 * |                     |              |fail if any bundles are currently pend|       |
 * |                     |              |ing acquisition via this induct.      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |induct_start         |44c1186506    |Start the indicated induct task as def|       |
 * |                     |              |ined for the indicated CL protocol on |       |
 * |                     |              |the local node.                       |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |induct_stop          |44c1186507    |Stop the indicated induct task as defi|       |
 * |                     |              |ned for the indicated CL protocol on t|       |
 * |                     |              |he local node.                        |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |manage_heap_max      |44c1186508    |Declare the maximum number of bytes of|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |outduct_add          |44c1186509    |Establish a duct for transmission of b|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |outduct_change       |44c118650a    |Set new values for the indicated duct'|       |
 * |                     |              |s payload size limit and the control t|       |
 * |                     |              |hat is used to initiate operation of t|       |
 * |                     |              |he outduct task for this duct.        |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |outduct_del          |44c118650b    |Delete the outduct identified by proto|       |
 * |                     |              |colName and ductName. The control will|       |
 * |                     |              | fail if any bundles are currently pen|       |
 * |                     |              |ding transmission via this outduct.   |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |outduct_start        |44c118650c    |Start the indicated outduct task as de|       |
 * |                     |              |fined for the indicated CL protocol on|       |
 * |                     |              | the local node.                      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |egress_plan_block    |44c118650d    |Disable transmission of bundles queued|       |
 ...
 * |                     |              |imbo.                                 |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |egress_plan_unblock  |44c118650e    |Re-enable transmission of bundles to t|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |outduct_stop         |44c118650f    |Stop the indicated outduct task as def|       |
 * |                     |              |ined for the indicated CL protocol on |       |
 * |                     |              |the local node.                       |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |protocol_add         |44c1186510    |Establish access to the named converge|       |
 ...
 * |                     |              |apped.                                |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |protocol_del         |44c1186511    |Delete the convergence layer protocol |       |
 * |                     |              |identified by protocolName. The contro|       |
 * |                     |              |l will fail if any ducts are still loc|       |
 * |                     |              |ally declared for this protocol.      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |protocol_start       |44c1186512    |Start all induct and outduct tasks for|       |
 * |                     |              | inducts and outducts that have been d|       |
 * |                     |              |efined for the indicated CL protocol o|       |
 * |                     |              |n the local node.                     |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |protocol_stop        |44c1186513    |Stop all induct and outduct tasks for |       |
 * |                     |              |inducts and outducts that have been de|       |
 * |                     |              |fined for the indicated CL protocol on|       |
 * |                     |              | the local node.                      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |scheme_add           |44c1186514    |Declares an endpoint naming scheme for|       |
 ...
 * |                     |              | status reports.                      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |scheme_change        |44c1186515    |Set the indicated scheme's forwarderCo|       |
 * |                     |              |ntrol and adminAppControl to the strin|       |
 * |                     |              |gs provided as arguments.             |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |scheme_del           |44c1186516    |Delete the scheme identified by scheme|       |
 * |                     |              |Name. The control will fail if any bun|       |
 * |                     |              |dles identified in this scheme are pen|       |
 * |                     |              |ding forwarding, transmission, or deli|       |
 * |                     |              |very.                                 |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |scheme_start         |44c1186517    |Start the forwarder and administrative|       |
 * |                     |              | endpoint tasks for the indicated sche|       |
 * |                     |              |me task on the local node.            |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |scheme_stop          |45c118651818  |Stop the forwarder and administrative |       |
 * |                     |              |endpoint tasks for the indicated schem|       |
 * |                     |              |e task on the local node.             |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |watch                |45c118651819  |Enable/Disable production of a continu|       |
 ...
 * |                     |              |t interval expired.                   |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |egress_plan_outduct_a|44c118651a    |Declare that the indicated convergence|       |
 * |ttach                |              |-layer protocol outduct is now a viabl|       |
 * |                     |              |e device for transmitting bundles to t|       | <<< ADDED
 * |                     |              |he node[s] identified by endpoint_name|       | (ETRI & HYU)
 * |                     |              |.                                     |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |egress_plan_outduct_d|44c118651b    |Declare that the indicated convergence|       |
 * |etach                |              |-layer protocol outduct is no longer a|       |
 * |                     |              | viable device for transmitting bundle|       | <<< ADDED
 * |                     |              |s to the node[s] identified by endpoin|       | (ETRI & HYU)
 * |                     |              |t_name.                               |       |
 * +---------------------+--------------+--------------------------------------+-------+
 */
...
#define DTN_ION_BPADMIN_CTRL_EGRESS_PLAN_OUTDUCT_ATTACH 0x1a	/* ETRI & HYU */
#define DTN_ION_BPADMIN_CTRL_EGRESS_PLAN_OUTDUCT_DETACH 0x1b	/* ETRI & HYU */
