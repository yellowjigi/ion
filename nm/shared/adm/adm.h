...
#include "../primitives/rules.h"//jigi, 08/16/20
...
int adm_add_sbr(ari_t *id, uvast start, expr_t *cond, uvast eval, uvast fire, ac_t *action);
//JIGI

ari_t *adm_build_ari(amp_type_e type, uint8_t has_parms, vec_idx_t nn, uvast id);
ari_t *adm_build_ari_lit_uint(uint32_t value);//JIGI
