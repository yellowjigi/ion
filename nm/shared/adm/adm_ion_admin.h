...
extern vec_idx_t g_dtn_ion_ionadmin_idx[11];
 ...
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |name                 |4480189600    |The human-readable name of the ADM.   |STR    |ion_admin               |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |namespace            |4480189601    |The namespace of the ADM.             |STR    |DTN/ION/ionadmin        |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |version              |4480189602    |The version of the ADM.               |STR    |v0.0                    |
 * +---------------------+--------------+--------------------------------------+-------+------------------------+
 * |organization         |4480189603    |The name of the issuing organization o|       |                        |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |clock_error          |4482188e00    |This is how accurate the ION Agent's c|       |
 * |                     |              |lock is described as number of seconds|       |
 * |                     |              |, an absolute value.                  |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |clock_sync           |4482188e01    |This is whether or not the the compute|       |
 * |                     |              |r on which the local ION node is runni|       |
 * |                     |              |ng has a synchronized clock.          |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |congestion_alarm_cont|4482188e02    |This is whether or not the node has a |       |
 * |                     |              |control that will set off alarm if it |       |
 * |                     |              |will become congested at some future t|       |
 * |                     |              |ime.                                  |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |congestion_end_time_f|4482188e03    |This is the time horizon beyond which |       |
 * |                     |              |we don't attempt to forecast congestio|       |
 * |                     |              |n                                     |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |consumption_rate     |4482188e04    |This is the mean rate of continuous da|       |
 * |                     |              |ta delivery to local BP applications. |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |inbound_file_system_o|4482188e05    |This is the maximum number of megabyte|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |inbound_heap_occupanc|4482188e06    |This is the maximum number of megabyte|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |number               |4482188e07    |This is a CBHE node number which uniqu|       |
 * |                     |              |ely identifies the node in the delay-t|       |
 * |                     |              |olerant network.                      |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |outbound_file_system_|4482188e08    |This is the maximum number of megabyte|       |
 * |                     |              |s of storage space in ION's local file|       |
 * |                     |              | system that can be used for the stora|       |
 * |                     |              |ge of outbound zero-copy objects. The |       |
 * |                     |              |default heap limit is 1 Terabyte.     |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |outbound_heap_occupan|4482188e09    |This is the maximum number of megabyte|       |
 * |                     |              |s of storage space in ION's SDR non-vo|       |
 * |                     |              |latile heap that can be used for the s|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |production_rate      |4482188e0a    |This is the rate of local data product|       |
 * |                     |              |ion.                                  |UINT   |
 * +---------------------+--------------+--------------------------------------+-------+
 * |ref_time             |4482188e0b    |This is the reference time that will b|       |
 * |                     |              |e used for interpreting relative time |       |
 * |                     |              |values from now until the next revisio|       |
 * |                     |              |n of reference time.                  |TV     |
 * +---------------------+--------------+--------------------------------------+-------+
 * |time_delta           |4482188e0c    |The time delta is used to compensate f|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |version              |4482188e0d    |This is the version of ION that is cur|       |
 ...
 * |        NAME         |     ARI      |             DESCRIPTION              | TYPE  |
 * +---------------------+--------------+--------------------------------------+-------+
 * |contacts             |448a189300    |This table shows all scheduled periods|       |
 * |                     |              | of data transmission.                |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |ranges               |448a189301    |This table shows all predicted periods|       |
 ...
 * |        NAME         |     ARI      |             DESCRIPTION              | TYPE  |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_init            |44c1188d00    |Until this control is executed, the lo|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_consumption_rate|44c1188d05    |This management control sets ION's exp|       |
 ...
 * |                     |              |forecasts.                            |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_contact_add     |44c1188d06    |This control schedules a period of dat|       |
 ...
 * |                     |              |tential contact.                      |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_contact_del     |44c1188d07    |This control deletes the scheduled per|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_inbound_heap_occ|44c1188d08    |This management control sets the maxim|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_outbound_heap_oc|44c1188d09    |This management control sets the maxim|       |
 * |                     |              |um number of megabytes of storage spac|       |
 * |                     |              |e in ION's SDR non-volatile heap that |       |
 ...
 * |                     |              |e.                                    |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_production_rate_|44c1188d0a    |This management control sets ION's exp|       |
 * |                     |              |ectation of the mean rate of continuou|       |
 ...
 * |                     |              | forecasts.                           |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_range_add       |44c1188d0b    |This control predicts a period of time|       |
 ...
 * |                     |              | seconds.                             |       |
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_range_del       |44c1188d0c    |This control deletes the predicted per|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_ref_time_set    |44c1188d0d    |This is used to set the reference time|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_time_delta_set  |44c1188d0e    |This management control sets ION's und|       |
 ...
 * +---------------------+--------------+--------------------------------------+-------+
 * |node_contact_change  |44c1188d0f    |This control changes the data transmis|       |
 * |                     |              |sion rate and possibly our level of co|       |
 * |                     |              |nfidence in the scheduled period of da|       | <<< ADDED
 * |                     |              |ta transmission from source_node to de|       | (ETRI & HYU)
 * |                     |              |st_node starting at start_time.       |       |
 * +---------------------+--------------+--------------------------------------+-------+
 */
...
#define DTN_ION_IONADMIN_CTRL_NODE_CONTACT_CHANGE 0x0f	/* ETRI & HYU */
