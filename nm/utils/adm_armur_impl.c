...
int kplo_install(uvast file_server_eid, char *file_server_file)
{
	...
	for (i = 0; i < file_count; i++)
	{
		...
		WE DON'T KNOW WHAT FILE NAME IT IS.
		- WE CANNOT STRCMP EVERY INSTANCE OF THE FILES WITH:
		BPTRANSIT, LIBBP.SO, LTPMETER, BPCLM, BPCHAT, ...
		- WE NEED TO RETRIEVE THE INFORMATION OF THE FILE FROM A HASH TABLE.

		RETRIEVE INFO OF /* THIS PROGRAM */ FROM A HASH TABLE.
		WHERE IS THE HASH TABLE PLACED?
		- SDR HEAP SPACE (NONVOLATILE)
