...
void dtn_ion_bpadmin_init_ctrl()
{
	...
	/* EGRESS_PLAN_OUTDUCT_ATTACH */	/* ETRI & HYU */

	id = adm_build_ari(AMP_TYPE_CTRL, 1, g_dtn_ion_bpadmin_idx[ADM_CTRL_IDX], DTN_ION_BPADMIN_CTRL_EGRESS_PLAN_OUTDUCT_ATTACH);
	adm_add_ctrldef_ari(id, 3, NULL);
	meta = meta_add_ctrl(id, ADM_ENUM_DTN_ION_BPADMIN, "egress_plan_outduct_attach", "Declare that the indicated convergence-layer protocol outduct is now a viable device for transmitting bundles to the node[s] identified by endpoint_name.");

	meta_add_parm(meta, "plan_name", AMP_TYPE_STR);
	meta_add_parm(meta, "protocol_name", AMP_TYPE_STR);
	meta_add_parm(meta, "duct_name", AMP_TYPE_STR);

	/* EGRESS_PLAN_OUTDUCT_DETACH */	/* ETRI & HYU */

	id = adm_build_ari(AMP_TYPE_CTRL, 1, g_dtn_ion_bpadmin_idx[ADM_CTRL_IDX], DTN_ION_BPADMIN_CTRL_EGRESS_PLAN_OUTDUCT_DETACH);
	adm_add_ctrldef_ari(id, 3, NULL);
	meta = meta_add_ctrl(id, ADM_ENUM_DTN_ION_BPADMIN, "egress_plan_outduct_detach", "Declare that the indicated convergence-layer protocol outduct is no longer a viable device for transmitting bundles to the node[s] identified by endpoint_name.");
