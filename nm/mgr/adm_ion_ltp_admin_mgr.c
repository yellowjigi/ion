...
vec_idx_t g_dtn_ion_ltpadmin_idx[11];
...
void dtn_ion_ltpadmin_init_ctrl()
{
	...
	id = adm_build_ari(AMP_TYPE_CTRL, 1, g_dtn_ion_ltpadmin_idx[ADM_CTRL_IDX], DTN_ION_LTPADMIN_CTRL_START);
	adm_add_ctrldef_ari(id, 1, NULL);
	meta = meta_add_ctrl(id, ADM_ENUM_DTN_ION_LTPADMIN, "start", "This control starts link service output tasks for all LTP spans (to remote engines) from the local LTP engine, and it starts the link service input task for the local engine.");

	meta_add_parm(meta, "lsi_control", AMP_TYPE_STR);
}
