...
void dtn_ion_ionadmin_init_ctrl()
{
	...
	/* NODE_CONTACT_CHANGE */	/* ETRI & HYU */
	
	id = adm_build_ari(AMP_TYPE_CTRL, 1, g_dtn_ion_ionadmin_idx[ADM_CTRL_IDX], DTN_ION_IONADMIN_CTRL_NODE_CONTACT_CHANGE);
	adm_add_ctrldef_ari(id, 5, NULL);
	meta = meta_add_ctrl(id, ADM_ENUM_DTN_ION_IONADMIN, "node_contact_change", "This control changes the data transmission rate and possibly our level of confidence in the scheduled period of data transmission from source_node to dest_node starting at start_time.");
