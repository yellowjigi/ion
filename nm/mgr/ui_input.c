...
char *   ui_input_string(char *prompt)
{
	...
	*/
 	...
 	/* 
	 * Scan every sequence of characters until 'LF(newline)' or 'NULL(\0)' is encountered
 	 * to support some cases where the input string includes one or more white-space characters,
	 * e.g., the 'LSO_command' parameter for Controls such as span_add and span_change.
	 */
	sscanf(line, "%[^\n]", result);
