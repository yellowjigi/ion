			...
			uint8_t targetBlockType, char *secSrcEid)
{
	...
	int			result;
	char			*fromEid;
	unsigned int		itemp;		/* ETRI & HYU */
	unsigned char		*cursor;	/* ETRI & HYU */
	int			unparsedBytes;	/* ETRI & HYU */
	...
	for (idx = 0; idx < 2; idx++)
	{
				...
				elt = lyst_next(elt))
		{
			...
			/*
			 * If the extension block to review here is a BIB,
			 * (that is what we are testing for the AMP-04 test)
			 * the ASB (blk->object) has not yet been deserialized
			 * by the sbsp_bibParse() function. Some relevant codes
			 * should be modified, but for now we apply just a
			 * simple method to retrieve only the targetBlockType
			 * of the ASB to go on to the following codes.
			 */
			if (blk->type == BLOCK_TYPE_BIB)
			{
				unparsedBytes = blk->length;
				cursor = ((unsigned char *)(blk->bytes))
					+ (blk->length - blk->dataLength);
				_extractSmallSdnv(&itemp, &cursor, &unparsedBytes, __LINE__);
			}

			asb = MTAKE(sizeof(SbspInboundBlock));
			if (asb == NULL)
			{
				return -1;
			}
